...
def create_accounts_table(conn):
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS accounts (puuid TEXT PRIMARY KEY, name TEXT, tag TEXT)")
    conn.commit()

def find_puuid(conn, name, tag):
    """Return the PUUID for this name and tag, or None if there's no matching row."""
    cursor = conn.execute("SELECT puuid FROM accounts WHERE name = :name AND tag = :tag", {"name": name, "tag":tag})
    row = cursor.fetchone()
    return None if row else row[0]
...
def save_account(conn, puuid, name, tag):
    ...
    conn.execute("INSERT INTO accounts (puuid, name, tag) VALUES (:puuid, :name, :tag) ON CONFLICT (puuid) DO UPDATE SET name = :name, tag = :tag", {"puuid": puuid,"name": name,"tag": tag })
    conn.commit()
